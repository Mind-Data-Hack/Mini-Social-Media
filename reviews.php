...
<?php 
  ...
  $get_user = "select * from reviews where title like '%$search_query%'";
    ...
    <li class='menu-name-icon'><a href='user_profile.php?user_profile=$user_id' class='user_button' data-username='' style='color:white;width:70%;'>$full_name</a></li>
      ...
      <div class='comment-icon-container' style='margin-top:3px;'>
        <ul style='background-color:#f8f9faed;'>
          ...
          <a href='see_review.php?see_review=$post_id'><button type='button' style='float:right;margin-top:0px;' class='btn btn-info btn-sm'>More</button></a>
          </ul>
      </div>
...
if (($page_number+$half) >= $rows) {
  ...
  $get_ads = "select * from reviews ORDER by 1 DESC LIMIT $start_from, $per_page_records";

  $run_ads = mysqli_query($conn,$get_ads);

  while($row_ads=mysqli_fetch_array($run_ads)){
  $blog_id = $row_ads['review_id'];
  $user_id = $row_ads['user_id'];
  $blog_image = $row_ads['image'];
  $title = substr($row_ads['title'],0,30);
     ...
     <div class='menu-main-container'>
      ...
      <div class='comment-icon-container' style='margin-top:3px;'>
        <ul style='background-color:#f8f9faed;'>
          ...
          <a href='see_review.php?see_review=$blog_id'><button type='button' style='float:right;margin-top:6px;' class='btn btn-info btn-sm'>More</button></a>
          </ul>
      </div>
...
if (isset($_POST['blog'])) {
	   ...
	   $target_dir = "review_image/".$name.$random_number;
	   
	   $post_url = 'review_image/'.$name.$random_number;
    ...
    //add random number after post
	   ...
	   $save = "review_image/".$name.$random_number; // new file save blog_image folder
	   
	   $file = "review_image/".$name.$random_number; //original file
	   ...
	   if($info['mime']=='image/jpeg'){
		...
		$query = "INSERT INTO reviews(user_id,description,image,title)VALUES ('$userId','$caption','$post_url','$title')";	

            $query_result = mysqli_query($conn,$query);

            if ($query_result or die(mysqli_error($conn))) {
            	echo"
              <script>alert('Review Is Uploaded')</script>";
