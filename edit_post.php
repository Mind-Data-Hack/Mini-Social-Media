...
<div class="col-sm-6">
	...
	if(isset($_GET['edit_post'])){
		...
		$get_post = "select * from blogs where blog_id='$get_id'";
		$run_post = mysqli_query($conn,$get_post);
		$row=mysqli_fetch_array($run_post);

		$post_con = $row['description'];
		$profile_image_url = $row['blog_image'];
		$title = $row['title'];
...
<?php 
		if(isset($_POST['update'])){
                ...
                $profile_image_url = 'blog_image/'.$name.$random_number;
...
if (empty($name)) {
	$profile_image_url = $row['blog_image'];
		...
		$update_post = "update blogs set description='$content',title='$title',blog_image='$profile_image_url'  where blog_id='$get_id'";
